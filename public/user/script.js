...
async function loadDestinations() {
    // 1. Get values from the HIDDEN inputs (Updated to match new HTML)
    ...
    if (maxPrice) url += `&maxPrice=${maxPrice}`; // Now sends 100, 300, 500, etc.
    ...
    try {
        ...
        if(grid) grid.innerHTML = `<div style="grid-column: 1/-1; text-align:center; padding:40px; color:#ff5a5f;">Failed to load data. Please try again.</div>`;
    }
}
...
function renderGrid(data) {
    ...
    grid.innerHTML = data.map(item => {
        ...
        // SAFE STRINGIFY: Fixes quote issues in onclick handlers
...
function openShareModal(item) {
    // If passed as string from HTML, parse it; otherwise use as object
...
async function toggleFavourite(btn, itemId) {
    // Prevent bubbling if clicking inside card
...
// --- 7. DETAILS MODAL (With Animation Logic) ---
...
function openModal(item) {
    ...
    // Handle object vs string passed from HTML
    const dest = (typeof item === 'string') ? JSON.parse(item) : item;
    // Prepare string for sharing button
    const safeItem = JSON.stringify(dest).replace(/"/g, '&quot;');

    const imgUrl = dest.images || 'https://via.placeholder.com/800x450';
    const mapUrl = `https://www.google.com/maps/search/?api=1&query=$?q=$${encodeURIComponent(dest.name + ' ' + dest.state + ' Malaysia')}`;

    // Reset animation by removing content briefly (optional, or handled by CSS keyframes on display:flex)
            ...
            <div class="modal-flex" style="display:flex; flex-wrap:wrap; gap:20px;">
                ...
                <div class="modal-main" style="flex:2; min-width:300px;">
                    ...
                    <div style="margin-top:20px; background:#f9f9f9; padding:15px; border-radius:8px;">
                        ...
                        <p style="margin:0; color:#555;">${dest.activities || "Sightseeing, Relaxation, Photography"}</p>
                    </div>
                </div>
...
// Handle option selection (Updates Hidden Inputs)
// Arguments: dropdownId (HTML ID), text (Visual), value (Data for API)
...
function renderPagination(total) {
    ...
    if(!nav) return;
    nav.innerHTML = '';
    // Limit pagination buttons if too many pages (simple logic)
